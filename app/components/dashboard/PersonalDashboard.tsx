...
const calculateSayDoRatio = (salesData: any[], marketingData: any[], projections: any) => {
  ...
  const metrics = {
    ...
    posts: {
      ...
      actual: thisWeekMarketing.reduce((acc, day) => acc + (day.posts || 0), 0)
    }
  };

  console.log('Lead Indicator Say/Do Metrics:', metrics);
  ...
  console.log('Say/Do Final Calculation:', {
    totalTarget,
    totalActual,
    ratio,
    breakdown: metrics
...
const calculateConsistencyScore = (data: any[]) => {
  ...
  console.log('Calculating consistency with last 7 days:', last7Days);
  ...
  console.log('Consistency score:', { consistentDays, score });
  return score;
};
...
const calculateMomentumScore = (data: any[]) => {
  ...
  console.log('Momentum calculation:', { last3Days, previous3Days });
  ...
  console.log('Momentum averages:', { currentAvg, prevAvg });
...
const calculateVelocityScore = (salesData: any[], marketingData: any[], personalData: any[]) => {
  const thisWeek = {
    ...
    personal: personalData.slice(-7).reduce((acc, curr) => acc + (curr.personalXP || 0), 0)
  };
  ...
  const lastWeek = {
    ...
    personal: personalData.slice(-14, -7).reduce((acc, curr) => acc + (curr.personalXP || 0), 0)
  };

  console.log('Velocity calculation:', { thisWeek, lastWeek });

  const thisWeekTotal = thisWeek.sales + thisWeek.marketing + thisWeek.personal;
  const lastWeekTotal = lastWeek.sales + lastWeek.marketing + lastWeek.personal;
...
export default function PersonalDashboard({ 
  ...
  console.log('Personal Dashboard Data:', data);
  ...
  const calculateMetrics = (): PersonalMetrics => {
    ...
    const personalXP = data.reduce((acc, curr) => acc + (Number(curr.personalXP) || 0), 0);
    const personalLevel = Math.floor(personalXP / 2000) + 1;
    
    const combinedLevel = Math.floor((salesLevel + marketingLevel + personalLevel) / 3);
    ...
    const velocityScore = calculateVelocityScore(salesData, marketingData, data);
      ...
      <div className="bg-gray-900 border border-red-500/20 rounded-lg p-6 mb-6">
        <div className="flex justify-between items-stretch w-full gap-4">
          <div className="flex gap-4 flex-1">
            ...
            <div className="flex-1 flex flex-col items-center bg-gray-800 rounded-lg p-6 border border-red-500/20">
              <span className="text-md text-gray-400">Personal</span>
              <span className="text-3xl text-red-500 font-bold">L{metrics.personalLevel}</span>
            </div>
          </div>
